...
chrome.runtime.onMessageExternal.addListener(
    function(request, sender, sendResponse) {
        log("Got message, sender url: " + sender.url + " request: " + request.action);
        ...
        if (request.action === "connect") {
            ...
            sendResponse({message: "starting to search for serial devices"});
        /* When commands to send to serial device */
        } else if (request.action === "commands") {
            log("Received commands: " + request.commands);
            if (connectionId != -1) {
                chrome.serial.send(connectionId, str2ab(request.commands), function() {});
                sendResponse({message: "successfully sent commands to serial device"});
            } else {
                log("Not connected to a serial device");
                sendResponse({message: "not connected to a serial device"});
            }
        } else {
